...
frp_require_allocator_url() {
  ...
  if [[ -z "$url" ]]; then
    ...
    echo "Set FRP_ALLOCATOR_URL or use the installer command generated by: sudo drlink create zero-touch (or sudo drlink create enrollment)." >&2
    exit 1
  fi
  if [[ "$url" == http://* ]]; then
