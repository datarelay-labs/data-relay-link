...
mkdir -p "$TREE/etc/frp" "$TREE/var/log/drlink"
python3 - "$TREE" <<'PY'
import json, sys
...
(root / 'var/lib/drlink/registry.json').write_text(json.dumps({'schema_version': 2, 'clients': {}}) + '\n')
...
path = Path(root) / 'var/lib/drlink/enrollments' / (eid + '.json')
...
if state == 'expired':
    ...
    ticket['bound_machine_id'] = 'machine00000001'
root = Path(root)
(root / 'var/lib/drlink/bootstrap' / (tid + '.json')).write_text(json.dumps(ticket, indent=2) + '\n')
(root / 'var/lib/drlink/enrollments' / (eid + '.json')).write_text(json.dumps(enroll, indent=2) + '\n')
PY
}
...
rows = elc.collect_logical_enrollments(
    root / 'var/lib/drlink/enrollments',
    root / 'var/lib/drlink/bootstrap',
    now,
)
...
[[ ! -f "$TREE/var/lib/drlink/enrollments/1111111111111111.json" ]] || fail "MANUAL_EXPIRED_PURGE"
...
[[ ! -f "$TREE/var/lib/drlink/enrollments/2222222222222222.json" ]] || fail "MANUAL_COMPLETED_PURGE"
...
[[ ! -f "$TREE/var/lib/drlink/enrollments/3333333333333333.json" ]] || fail "MANUAL_REVOKED_PURGE"
...
[[ ! -f "$TREE/var/lib/drlink/bootstrap/4444444444444444.json" ]] || fail "ZERO_TOUCH_EXPIRED ticket"
[[ ! -f "$TREE/var/lib/drlink/enrollments/5555555555555555.json" ]] || fail "ZERO_TOUCH_EXPIRED enroll"
...
[[ ! -f "$TREE/var/lib/drlink/bootstrap/6666666666666666.json" ]] || fail "ZERO_TOUCH_COMPLETED ticket"
[[ ! -f "$TREE/var/lib/drlink/enrollments/7777777777777777.json" ]] || fail "ZERO_TOUCH_COMPLETED enroll"
...
[[ ! -f "$TREE/var/lib/drlink/bootstrap/8888888888888888.json" ]] || fail "ZERO_TOUCH_REVOKED ticket"
[[ ! -f "$TREE/var/lib/drlink/enrollments/9999999999999999.json" ]] || fail "ZERO_TOUCH_REVOKED enroll"
...
[[ ! -f "$TREE/var/lib/drlink/enrollments/${OLD_ID}.json" ]] || fail "AUTO_CLEANUP old terminal"
[[ -f "$TREE/var/lib/drlink/enrollments/${RECENT_ID}.json" ]] || fail "AUTO_CLEANUP recent terminal"
[[ -f "$TREE/var/lib/drlink/bootstrap/d1d1d1d1d1d1d1d1.json" ]] || fail "AUTO_CLEANUP active bootstrap"
NEW_ID="$(awk '/^Enrollment ID:/{print $3; exit}' "$WORK/auto.out")"
[[ -f "$TREE/var/lib/drlink/enrollments/${NEW_ID}.json" ]] || fail "AUTO_CLEANUP active pending"
pass "AUTO_CLEANUP"
...
path.write_text(json.dumps(rec, indent=2) + '\n')
ep = root / 'var/lib/drlink/enrollments' / 'eeeeffffaaaabbbb.json'
...
[[ -f "$TREE/var/lib/drlink/bootstrap/aaaabbbbccccdddd.json" ]] || fail "AMBIGUOUS_PAIR_PRESERVED ticket"
[[ -f "$TREE/var/lib/drlink/enrollments/eeeeffffaaaabbbb.json" ]] || fail "AMBIGUOUS_PAIR_PRESERVED enroll"
...
elc = importlib.util.module_from_spec(spec)
spec.loader.exec_module(elc)
cfg = json.loads((root / 'etc/drlink/config.json').read_text())
...
[[ -f "$TREE/var/lib/drlink/enrollments/1414141414141414.json" ]] || fail "NONINTERACTIVE preserved"
...
[[ ! -f "$TREE/var/lib/drlink/enrollments/1212121212121212.json" ]] || fail "BULK_PURGE"
pass "BULK_PURGE"
...
from pathlib import Path
root = Path(sys.argv[1])
audit = root / 'var/log/drlink/audit.log'
...
(root / 'var/lib/drlink/enrollments' / (eid + '.json')).write_text(json.dumps(rec) + '\n')
PY
...
for rel in ('var/log/drlink/audit.log', 'var/lib/drlink/audit.log'):
    p = root / rel
    if p.is_file():
...
from pathlib import Path
root = Path(sys.argv[1])
path = root / 'var/lib/drlink/enrollments' / '1818181818181818.json'
rec = json.loads(path.read_text())
rec['expires_at'] = int(time.time()) - 5 * 86400
path.write_text(json.dumps(rec, indent=2) + '\n')
PY
...
[[ ! -f "$TREE/var/lib/drlink/enrollments/1616161616161616.json" ]] || fail "BACKUP_CLEANUP old"
...
[[ -f "$TREE/var/lib/drlink/enrollments/1616161616161616.json" ]] || fail "BACKUP_RESTORE before"
# Restore post-cleanup backup → old record stays absent
python3 "$RESTORE" "$WORK/after.tar.gz" >/dev/null
[[ ! -f "$TREE/var/lib/drlink/enrollments/1616161616161616.json" ]] || fail "BACKUP_RESTORE after absent"
[[ -f "$TREE/var/lib/drlink/enrollments/1818181818181818.json" ]] || fail "BACKUP_RESTORE recent retained"
pass "BACKUP_RESTORE"
